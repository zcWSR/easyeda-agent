...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		{
			Name:        "schematic.attribute.visibility.modify",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Modify only visibility flags on an identified Name or Description attribute owned by a net-port, part, or sheet; guards exact parent/type/key/value/prior visibility and verifies readback.",
			Inputs:      []string{"parentPrimitiveId", "attributePrimitiveId", "expectedParentType", "expectedKey", "expectedValue", "expectedKeyVisible", "expectedValueVisible", "keyVisible optional", "valueVisible optional"},
			Outputs:     []string{"parentPrimitiveId", "attributePrimitiveId", "parentType", "key", "value", "keyVisible", "valueVisible", "verified"},
			VerifyWith:  []string{"schematic.components.list", "schematic.export.image"},
		},
		{
			Name:        "schematic.attribute.geometry.modify",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
		...
		{
			Name:        "library.footprint.reload",
			Domain:      DomainLibrary,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Save, close and reopen ONE exact active library footprint without replaying build — the typed persistence check for library authoring (the project-level doc reload only knows schematic/PCB and refuses library documents). Requires the target footprint to be the active library document; saves FIRST and never closes when the save fails, reopens the same asset, then compares the pad/polyline/fill/region primitive-ID inventories before/after and reports verified=false (with a warning) when they differ. Structured partial result instead of a throw once the tab has been closed.",
			Inputs:      []string{"uuid", "libraryUuid"},
			Outputs:     []string{"saved", "closed", "reopened", "before", "after", "verified", "partial/priorTabId/error on failure"},
		},
		{
		...
		{
			Name:        "pcb.silk.artwork_list",
			Domain:      DomainPcb,
			Phase:       2,
			NeedsWindow: true,
			Description: "List every FREE silkscreen artwork primitive on the active PCB with its exact primitiveId, kind (image|fill|line|arc|polyline|string), silk layer and rendered bbox. This is the scoped-delete companion of pcb.silk.artwork_delete and the way to find the previous generation of `silk-zone-outline` / `silk-import-svg` artwork after a partial redraw. Component designator/value ATTRIBUTES are excluded on purpose: they are owned by their parent part and are not free artwork. Layer filter 3=TOP_SILKSCREEN, 4=BOTTOM_SILKSCREEN; an unreadable primitive class fails the whole listing instead of returning a partial inventory. Read-only.",
			Inputs:      []string{"layer optional (3|4 — omit for both)"},
			Outputs:     []string{"items[].primitiveId", "items[].kind", "items[].layer", "items[].locked", "items[].bbox", "items[].text (strings only)", "count", "layers", "scope"},
			VerifyWith:  []string{"pcb.silk.artwork_list", "pcb.snapshot"},
		},
		{
			Name:        "pcb.silk.artwork_delete",
			Domain:      DomainPcb,
			Phase:       2,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Delete ONLY explicitly named FREE silkscreen artwork primitive ids (the scoped undo for pcb.silk.import_svg / pcb silk-zone-outline). All-or-nothing preflight: every id must still be present as UNLOCKED free silk artwork on layer 3/4 according to a FRESH enumeration, otherwise nothing is deleted (component designator/value attributes and non-silk ids are refused). Deletion is grouped per primitive class, then the inventory is re-read: survivors are reported structurally with a do-not-replay warning, and a total no-op is a hard error because the canvas is provably unchanged. Always verify with pcb.silk.artwork_list and pcb check after save/reload.",
			Inputs:      []string{"primitiveIds (required: non-empty string array of ids from pcb.silk.artwork_list)"},
			Outputs:     []string{"requested", "deleted", "count", "errors", "verified", "partial/survivors on partial"},
			VerifyWith:  []string{"pcb.silk.artwork_list", "pcb.snapshot"},
		},
		{
			Name:        "pcb.nets.list",
