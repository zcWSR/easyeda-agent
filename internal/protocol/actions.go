...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		},
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Move or rotate only the native visible Name attribute owned by an exact wire. Requires parent wire id/net/full line and attribute id/key/value/old pose/visibility; verifies unchanged wire geometry, network and label text after write.",
			Inputs:      []string{"parentPrimitiveId", "attributePrimitiveId", "expectedParentType", "expectedKey", "expectedNet", "expectedLine", "expectedValue", "expectedX", "expectedY", "expectedRotation", "expectedKeyVisible", "expectedValueVisible", "x optional", "y optional", "rotation optional"},
