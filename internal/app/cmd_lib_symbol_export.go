...
func newLibrarySymbolExportSourceCmd(cfg *appConfig, stdout, stderr io.Writer, window *string) *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Example: `  easyeda lib symbol export-source --uuid <sheet-symbol-uuid> --library <library-uuid>
  easyeda lib symbol export-source --uuid <sheet-symbol-uuid> --library <library-uuid> --out sheet-symbol.elibz2`,
