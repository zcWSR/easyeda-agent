...
func TestWindowsListenerPID(t *testing.T) {
	base := " TCP  0.0.0.0:60832  0.0.0.0:0 LISTENING 456\r\n TCP [::]:60832 [::]:0 LISTENING 456\n TCP 127.0.0.1:60832 127.0.0.1:50000 ESTABLISHED 999\n TCP 0.0.0.0:160832 0.0.0.0:0 LISTENING 777"
	if n := windowsListenerPID(base, 60832); n != 456 {
		t.Fatalf("got %d", n)
	}
	if n := windowsListenerPID(base+"\n TCP 127.0.0.1:60832 0.0.0.0:0 LISTENING 789", 60832); n != 0 {
...
func TestDaemonLifecycleRefusesRemoteHostAndInvalidPID(t *testing.T) {
	for _, host := range []string{"192.0.2.1", "example.com", "0.0.0.0", "::"} {
		if stopLocalDaemon(host, 60832, io.Discard) == nil {
...
func TestDaemonLifecycleHelper(t *testing.T) {
	...
	http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		case "unknown":
			fmt.Fprint(w, `{"service":"easyeda-agent","pid":-1}`)
		case "old":
			fmt.Fprint(w, `{"service":"easyeda-agent"}`)
		default:
