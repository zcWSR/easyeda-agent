...
func newProjectExportSourceCmd(cfg *appConfig, stdout, stderr io.Writer, window *string) *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Example: `  easyeda project export-source --uuid <current-project-uuid> --window <window-id>
  easyeda project export-source --uuid <current-project-uuid> --out project.epro2`,
