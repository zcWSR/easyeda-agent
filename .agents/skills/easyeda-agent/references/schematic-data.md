...
布局前对目标当前页运行 `easyeda sch designator-geometry --project <project> --doc <page> --out designators.json`。
...
稳定网名顺序；这些是共享预算内的有限重排，不是无限回溯。同一线树比较
...
合法标签错长。命令不写 EDA；执行使用 `easyeda sch apply`，写后回读、显式保存和官方导图。
