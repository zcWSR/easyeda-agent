...
- `easyeda pcb silk-art-list` / `easyeda pcb silk-art-delete` — **自由丝印的受控枚举与删除（v1.6.0-dev.7 起）**。`easyeda pcb silk-zone-outline` 每个功能区生成 **一个填充的 `pcb_PrimitiveImage`**（不是导线、不是 fill、也不进 `pcb silk-list`），区域名是**另一条** string —— 重画撤销旧轮廓时 `pcb fill list` / `pcb silk-list` 都找不到，这是实测踩过的坑。`silk-art-list [--layer 3|4]` 列出全部**自由**丝印图元（image / fill / line / arc / polyline / string，含 primitiveId、kind、layer、locked、真实 rendered bbox），用 bbox 分辨叠在一起的两代轮廓；`silk-art-delete --ids id1,id2` 只删点名 ID：写前整批预检（必须是仍在 layer 3/4 上、未锁定的自由丝印；designator/value 属性和非丝印 ID 一律拒绝，整批不删），写后重新枚举——存活项结构化返回并警告不要重放，全部存活则硬报错（画布可证未变）。`silk-zone-outline --replace-ids <旧轮廓 ID>` 是幂等重画：**先建新轮廓，全部成功后才删旧 ID**，失败最坏留下重复而不会留下空白区域；`--dry-run` 与 `--replace-ids` 互斥。删除后仍要 save → reload → `silk-art-list` / `pcb check` 回读；`pcb clear --only silk` 依旧只是整板级重来。
...
- `easyeda pcb layout-lint` — **placement and routability facts before routing**。`--min-gap` 默认使用电气 clearance；手焊样例可通过 `pcb stage set-assembly --profile hand-solder` 提供装配参数，并检查普通间距、tight pair 与 solder-access。每个器件 bbox 四侧至少一侧有 `largePadAccessMil`（默认 60mil）的净通道；四面被围报告 `no-access`。`--gate`、`pre_route_passed` 和 confirm 状态仅为旧脚本兼容，不解锁布线。v1 仍是器件 bbox 级近似，Type-C 外壳脚/SOT-223 的实际烙铁进入方向须结合焊盘和局部图复核。
